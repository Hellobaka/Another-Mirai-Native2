...
namespace Another_Mirai_Native.Abstractions.Handlers
{
    ...
    {
        /// <summary>
        /// 在收到群邀请加入群时触发。可以通过重写此方法来处理添加请求，例如自动接受、拒绝或忽略请求。不重写时默认行为是忽略请求，即不做任何处理。处理请求请调用参数的 <see href="GroupAddRequestContext.SetRequestResult"/> 方法
        /// </summary>
        ...
        Task<EventHandleResult> OnGroupInviteRequestAsync(GroupInviteRequestContext e, CancellationToken ct);
    }
}
