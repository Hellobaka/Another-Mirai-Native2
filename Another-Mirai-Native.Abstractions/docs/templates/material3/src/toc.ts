...
export async function renderToc(): Promise<TocNode[]> {
  ...
  function renderTocNodes(nodes: TocNode[]): TemplateResult {
    const result = nodes.map(node => {
      ...
      const dom = href
        ? html`<a class='${classMap({ 'md-toc-link': !activeNodes.includes(node) })}' href=${href}>${breakWordLit(name)}</a>`
        : (isLeaf
            ? html`<span class='name-only'>${breakWordLit(name)}</span>`
            : html`<a class='${classMap({ 'md-toc-link': !activeNodes.includes(node) })}' href='#' @click=${toggleExpand}>${breakWordLit(name)}</a>`)
  ...
  function renderTocFilter(): TemplateResult {
    return disableTocFilter
      ...
      <form class='filter'>
        ...
        <input @input=${filterToc} value='${tocFilter}' type='search' placeholder='${loc('tocFilter')}' autocomplete='off' aria-label='${loc('tocFilter')}'>
      </form>`
...
function renderNextArticle(items: TocNode[], node: TocNode) {
  ...
  const prevButton = prev ? html`<div class="prev"><span><span class="material-symbols-outlined">chevron_left</span> ${loc('prevArticle')}</span> <a href="${prev.href}" rel="prev">${breakWordLit(prev.name)}</a></div>` : null
  const nextButton = next ? html`<div class="next"><span>${loc('nextArticle')} <span class="material-symbols-outlined">chevron_right</span></span> <a href="${next.href}" rel="next">${breakWordLit(next.name)}</a></div>` : null
