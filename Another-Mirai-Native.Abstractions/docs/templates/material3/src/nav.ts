...
export async function renderNavbar(): Promise<NavItem[]> {
  ...
  const menuItem = (item: NavItem) => {
    ...
    return html`<li class='md-nav-item'><a class='md-nav-link ${active}' aria-current=${current} href=${item.href}>${breakWordLit(item.name)}</a></li>`
  }

  const menu = html`
    <ul class='md-navbar'>${navItems.map(item => {
    if ('items' in item) {
      const active = item.items.some(i => i === activeItem) ? 'active' : null
      return html`
            <li class='md-nav-item md-nav-dropdown'>
              <a class='md-nav-link ${active}' href='#' @click=${(e: Event) => e.preventDefault()}>
                ${breakWordLit(item.name)}
                <span class="material-symbols-outlined">expand_more</span>
              </a>
              <ul class='md-dropdown-menu'>${item.items.map(menuItem)}</ul>
            </li>`
    } else {
  ...
  async function renderCore() {
    const icons = html`
      <div class="md-top-app-bar-icons">
        ${iconLinks?.map(i => html`
          <md-icon-button href="${i.href}" title="${i.title || ''}" aria-label="${i.title || ''}">
    ...
    render(
      html`
        <ol class="md-breadcrumb">
          ${breadcrumb.map(i => html`<li class="md-breadcrumb-item"><a href="${i.href}">${breakWordLit(i.name)}</a></li>`)}
        </ol>`,
      container)
  }
}
...
function inThisArticle(): TemplateResult {
  ...
  if (headings.length > 0) {
    return html`
      <h5 class="md-typescale-label-large">${loc('inThisArticle')}</h5>
      <ul>${headings.map(h => h.tagName === 'H2'
        ? html`<li><a class="link-primary" href="#${h.id}">${breakWordLit(h.textContent)}</a></li>`
